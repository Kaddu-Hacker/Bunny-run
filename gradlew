...
if [ "$cygwin" = "true" -o "$msys" = "true" ] ; then
    ...
    OURCYGPATTERN="$OURCYGPATTERN|(^)(/)(usr)(/)(local)(/)(.*)"
    OURCYGPATTERN="$OURCYGPATTERN|(^)($TEMP)(/)(.*)"
    OURCYGPATTERN="$OURCYGPATTERN|(^)($TMPDIR)(/)(.*)"
    for arg in "$@" ; do
        if [ "$arg" =~ $OURCYGPATTERN ] ; then
            arg=`cygpath --path --unix "$arg"`
        fi
        # Roll the args list so we have the last one in the position to return
        shift
        set -- "$@" "$arg"
    done
fi
